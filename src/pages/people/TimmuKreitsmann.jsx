...
export default function TimmuKreitsmann() {
  // importing data for publications
  const [publicationList, setPublicationList] = useState([]);
  const publicationCollectionRef = collection(
    db,
    "Publications/Timmu Kreitsmann/Timmu Kreitsmann Publications"
  );
  // 

  // reference to image from firebase storage
  const [imageURL, setImageURL] = useState(null);
  const storageRef = ref(storage, "Profile Images/Timmu Kreitsmann");
    ...
    <>
      <div className="AboutMeContainer">
        ...
        <div className="PersonalInfo">
          <h1>Timmu Kreitsmann</h1>
          <p>{peoplePage.timmuKreitsmann}</p>
        </div>
      </div>
